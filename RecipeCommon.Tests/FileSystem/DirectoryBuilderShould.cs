...
public class DirectoryBuilderShould
{
    ...
    public static IEnumerable<object[]> GetEqualityTestingData()
    {
        yield return new object[]
        {
            new FileContentMapping("folder1", builder => builder.WithText("Hello world")),
            true
        };

        yield return new object[]
        {
            new FileContentMapping("folder2", builder => builder.WithText("Hello world")),
            false
        };

        yield return new object[]
        {
            new FileContentMapping("123456", builder => builder.WithText("Hello world")),
            false
        };

        yield return new object[]
        {
            new FileContentMapping("hello", builder => builder.WithText("Hello world")),
            false
        };
    }

    [Theory]
    [MemberData(nameof(GetEqualityTestingData))]
    public void ReturnsCorrectEqualityForMapping(FileContentMapping mapping, bool expected)
    {
        FileContentMapping testing = new FileContentMapping("folder1", null);

        bool actual = testing.Equals(mapping);
